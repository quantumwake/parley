...
rows ([FEATURES.md](02_features.md)).
Companions: [RFC-0001](../rfcs/RFC-0001-conversation-log-platform.md) (storage
mapping, decisions), [CONCEPTUAL.md](../architecture/03_conceptual.md) (architecture),
[CONCEPTUAL-PRODUCT-REQUIREMENT.md](03_requirements.md)
...
Same envelope; `identity` is set on every event in every
...
| 3.2 Publish | `statefs-ai conversation post` and MCP `conversation.publish` | message per §2.2; `identity` from the writer's key, `participant` from the subscription handle; threads by `reply_to` |
