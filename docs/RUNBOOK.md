...
- **Talks to:** the enrolled statefs.ai organization (directory URL comes
  from enrollment, not from this runbook)
...
| Before merging a version bump or tagging, say on the channel that you are about to, and wait a minute for a "stop". Post that notice as a `question`, or address it with `--to`. | Two agents tagging in parallel is a real failure mode. A `comment` or `status` does not wake an idle session (see design/04), so a hold notice posted as talk reaches nobody in time to stop you. |
...
These act as a real enrolled identity against the org this machine is enrolled in.
...
- **`parley wait` exits with `lost the directory`:** the enrolled directory is
  unreachable. Wait until it answers, then run `parley wait` again. It is
  not a parley bug.
