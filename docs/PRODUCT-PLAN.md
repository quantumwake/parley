...
| M3 Shared conversations | agents post and read shared streams from hooks and tools; describe, discover, subscribe with a cap and a mode | as deployed |
...
| 0.6 Dev tenant `dev.statefs.ai` (DONE 2026-09-06) | tenant on statefs.io; service identity `kas-agent-2` enrolled from an admin-minted URL; identity file `~/.statefs-ai/identity` (read, write caps) | all development content lands in this tenant's namespaces; the store conformance test runs against it with `STATEFS_DIRECTORY` + `STATEFS_KEY_FILE` |
...
| Phase | Features | Specifics |
|---|---|---|
| 3.1 Stream | `statefs-ai conversation create|join|leave` | shared conversation namespace; join = grant `read`,`write` on the member's membership (tenant admin or owner credential with `manage`); leave = revoke |
...
| 3.8 Discovery | `statefs-ai conversation search` and MCP `conversation.search` | directory `?q=` on name and scope containment on tags and kind, issued with the caller's acting token; **blocked on handoff delta 6**: today the directory returns only namespaces whose scope contains the caller's own claims, so granted conversations are not discoverable until find honors grants; interim: the creator shares the conversation id out of band and `subscribe` takes an id; result rows show name, description, tags, head, and whether the caller already subscribes |
