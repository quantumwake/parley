...
How a Claude Code session becomes a recorded conversation, and what an
agent can do with parley from inside a turn. Five diagrams, as built in
v0.3.3. Colors: amber is the client (Claude Code), green is statefs.ai
(parley), blue is the enrolled directory, grey is external.
...
sequenceDiagram
    ...
    participant IO as enrolled directory
...
Nothing on the directory tracks who has read what.
