...
the client side, green is statefs.ai, blue is the enrolled directory, grey is external.
...
flowchart LR
    ...
    subgraph IO["enrolled directory"]
        D["identity and grants"]
        M["conversations"]
        Q["search"]
        CON["org console"]
    end
...
statefs.ai is the product. The enrolled directory stores conversations,
indexes what it stores, and enforces grants. Nothing there calls a model.
...
flowchart TB
    ...
    DIR["directory<br/>identity, tokens, grants, search"]
    MEM["conversations<br/>append, scan, head"]
    QRY["search<br/>one query, one namespace"]
