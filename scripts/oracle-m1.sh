...
# Against a live tenant:  STATEFS_DIRECTORY=<directory url> scripts/oracle-m1.sh
...
rm -rf "$DATA/spool" "$DATA/daemon.log" "$DATA/hooks.log"; rm -f "$DATA"/daemon-*.pid 2>/dev/null || true
echo "store: ${STATEFS_AI_STORE:-directory at $STATEFS_DIRECTORY}"
claude -p "Run the shell command echo m1-oracle, then reply with one sentence describing what you did." \
  --plugin-dir . --model haiku --max-turns 3 --allowedTools "Bash(echo:*)" --output-format text < /dev/null >/dev/null
...
NAME=$(ls -t "$DATA"/names/ | head -1 | python3 -c "import sys,urllib.parse;print(urllib.parse.unquote(sys.stdin.read().strip()))")
